...
18. **Release safeguards:** verified campaign backup/restore, strict save migrations, direct Pages preference bridging, production serving of the complete Godot payload, synchronized `docs/` and `web/public/game/` payloads, the local beta release gate, and CI native-economy plus Web/parity checks prevent stale, inaccessible, economically broken, or mismatched deployments. CI pins the official Godot 4.7 Windows archive by SHA-256 and uploads the bounded native shard evidence.
...
The gate requires Node 22.13 or newer. When more than one Node installation is present, pass its folder with `-NodeDirectory`. The gate writes a machine-readable report to `output/release/beta-release-gate.json`. Its native release set includes the campaign balance matrix, save store, semantic recovery, checkpoint coordination, operations economy, and persistence; the full isolated Godot suite remains available through `tools/run_godot_full_suite.ps1` for release candidates.

Generate the exact-candidate readiness manifest and one-page blocker dashboard
after every export, gate run, commit, or evidence update:

```powershell
./tools/write_release_readiness_report.ps1
```

The JSON report records the current commit, authoritative PCK identity, all nine
`docs/` versus `web/public/game/` payload comparisons, all six shipping PCK
locations, automated-gate freshness, and both seven-session evidence matrices.
Its Markdown companion lists only current blockers and their exact next
actions. Pass `-RequireReleaseReady` in a publication workflow to fail closed
when the worktree is not frozen, a payload differs, automation is stale, an
evidence record targets another candidate, or any required session is pending.
The beta gate exercises this reporting contract without converting legitimate
external blockers into fabricated passes.
