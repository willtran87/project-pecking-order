...
test("keeps the full probation-to-Senior reference in a collapsed responsive handbook", async () => {
  ...
  assert.match(html, /specialty and current file/);
	...
	assert.match(html, /manual tray is an explicit override that engages her disclosed work style/);
	assert.match(html, /temperament can trade pace, shell safety, strain, or recovery/);
	assert.match(html, /Open FILE before 55% completion/);
	assert.match(html, /what happened, what they need, and what delay costs them/);
	assert.match(html, /Settlement, Fast Denial, or Coverage Exception/);
	assert.match(html, /Nest paths can restore handler morale, Predator paths carry extra stress, and Appeals paths change audit order/);
	assert.match(html, /returns the same claimant to tomorrow(?:'|&#x27;)s Appeals tray/i);
  ...
  assert.match(html, /imperfect route/);
	...
	assert.match(html, /Flockwatch Capital for the exact run rate, cost breakdown, next Farm Mutual market/i);
	assert.match(html, /Capital also holds the Blueprint and Campus Portfolio for visible office expansion/i);
