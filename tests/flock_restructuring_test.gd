...
func _run() -> void:
	...
	if not failures.is_empty():
		...
		quit(1)
		return
	print("FLOCK_RESTRUCTURING_TEST_PASSED candidate=bottom choices=exact paid=atomic replacement=minimum persistence=v7-v4 endings=distinct")
	quit(0)
...
func _test_v6_json_round_trip_and_v4_neutral_migration(failures: Array[String]) -> void:
	...
	_check(int(exported.get("state_version", -1)) == 7, "restructuring checkpoint should use simulation schema v7", failures)
	for field in RESTRUCTURING_V5_FIELDS:
		_check(exported.has(field), "schema v7 should export restructuring field %s" % field, failures)
