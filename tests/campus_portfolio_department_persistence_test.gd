...
func _test_schema_v21_neutral_migration(failures: Array[String]) -> void:
	...
	_check(int(migrated.get("state_version", -1)) == 25, "v21 checkpoint should re-export as schema v25", failures)
...
func _test_portfolio_round_trip_and_strict_tamper_rejection(failures: Array[String]) -> void:
	...
	_check(int(valid.get("state_version", -1)) == 25 and valid.has("campus_portfolio"), "staffed portfolio checkpoint should export the v25 root", failures)
