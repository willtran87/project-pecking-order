...
func _test_full_campus_and_forty_eight_lots_round_trip(failures: Array[String]) -> void:
	...
	_check(int(exported.get("state_version", -1)) == DepartmentSimulation.SAVE_STATE_VERSION, "campus checkpoint should export the current schema v23", failures)
	_check(exported.has("campus_expansion"), "schema v23 should persist campus expansion root", failures)
...
func _test_schema_v20_neutral_migration(failures: Array[String]) -> void:
	...
	_check(int(migrated.get("state_version", -1)) == DepartmentSimulation.SAVE_STATE_VERSION, "v20 checkpoint should re-export as the current schema v23", failures)
