...
func _run() -> void:
	...
	_check(
		int(micro.get("budget_seconds", 0)) == 90
		...
		"the local first-session funnel should resolve plan, route, react, and reward inside one 90-second micro-shift contract",
		failures,
	)
	...
	if not failures.is_empty():
		...
		quit(1)
		return
	print("COMPLETE_GAME_LOOP_TEST_PASSED first_shift=90s rhythm=6-stage explain=4-chip trails=world stories=3-beat report=3-card replay=fast items=24")
	quit(0)
